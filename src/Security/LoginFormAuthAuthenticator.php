...

{
    ...
    {
        if ($user->getPassword()==$credentials['mdp']){
            return true;
        }
        else {
            return false;
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        if(in_array('ROLE_EMP',$user->getRoles(),true)) {
            return new RedirectResponse($this->urlGenerator->generate('app_formation_emp'));
        }
        if(in_array('ROLE_CANDIDATE',$user->getRoles(),true)) {
            return new RedirectResponse($this->urlGenerator->generate('app_main_can',["idcan"=>$user->getUserIdentifier()]));
        }
